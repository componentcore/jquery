(function($){
	...
	// blurbtitle (became videoTitle), pageurl (became pageUrl), googleToken (became token)
	$.fn.youTubeEmbed = function(settings){
		...
		// Auto-play - not sure where to put this
		...
		$.get(youtubeAPI,{'q':settings.videoID},function(response){
			...
			// Temp - until script added for scaling.
			if (currentWidth>470) {currentWidth = 470}

			//alert('settings.width ' + settings.width);

			settings.height = Math.round(currentWidth*settings.ratio);

			//alert('settings.height' + settings.height);
					...
					if(!initialized)
					{
						...
						elements.control.click(function(){
							if(!elements.container.hasClass('playing')){
								...
								if (settings.token) {
									// googleToken not currently used here since _gaq resides in calling page.
									_gaq.push(['_trackEvent', 'Play', settings.videoID, settings.video]);
								}
								//alert('videoID: ' + settings.videoID);
								//alert('video: ' + settings.video);
								//alert('videTitle: ' + settings.videoTitle + ' pageUrl: ' + settings.pageUrl);
